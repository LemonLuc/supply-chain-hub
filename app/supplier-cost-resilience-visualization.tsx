import {
  supplierCostResilienceActions,
  supplierCostResilienceBands,
  type ResolvedSupplierCostResilienceItem,
  type SupplierCostResilienceAction,
  type SupplierCostResilienceBand,
  type SupplierCostResilienceVisualization as Visualization,
} from "@/lib/supplier-cost-resilience";

const resilienceOrder: SupplierCostResilienceBand[] = ["High", "Medium", "Low"];

function actionSlug(action: SupplierCostResilienceAction): string {
...
function ActionLegend({ suppliers }: { suppliers: ResolvedSupplierCostResilienceItem[] }) {
  const visibleActions = supplierCostResilienceActions.filter((action) =>
  ...
  return (
    <ul className="portfolio-action-legend" aria-label="Decision action legend">
      {visibleActions.map((action) => (
        <li key={action}>
          <span
            className={`portfolio-action-swatch action-${actionSlug(action)}`}
            aria-hidden="true"
          />
          {action}
        </li>
...
function SupplierMarker({ supplier }: { supplier: ResolvedSupplierCostResilienceItem }) {
  return (
...
function SupplierMatrix({ suppliers }: { suppliers: ResolvedSupplierCostResilienceItem[] }) {
  return (
    <div
      ...
      <table className="portfolio-matrix" aria-label="Supplier cost and resilience matrix">
        <thead>
          <tr>
            ...
            {supplierCostResilienceBands.map((cost) => (
            ...
            <tr key={resilience}>
              ...
              {supplierCostResilienceBands.map((cost) => {
                const cellSuppliers = suppliers.filter(
                  (supplier) =>
                    supplier.cost === cost && supplier.resilience === resilience,
                );
                ...
                return (
                  ...
                  >
                    {cellSuppliers.length > 0 ? (
                      ...
                      <span
                        className="portfolio-empty-cell"
                        aria-label="No suppliers in this segment"
                      >
                        —
                      </span>
                    )}
...
function bubbleRadius(
  supplier: ResolvedSupplierCostResilienceItem,
...
function SupplierBubbleChart({ suppliers }: { suppliers: ResolvedSupplierCostResilienceItem[] }) {
  return (
      ...
      >
        ...
        <g className="portfolio-bubbles">
          {suppliers.map((supplier) => {
            ...
            return (
              <g
                className={`portfolio-bubble action-${actionSlug(supplier.action)}`}
...
export function SupplierCostResilienceVisualization({
  visualization,
}: {
  ...
  return (
    <section
      className="supplier-portfolio-section supplier-cost-resilience-section"
      aria-labelledby="supplier-portfolio-title"
    >
